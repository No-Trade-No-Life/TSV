...
在 Chromium 系浏览器中，可通过“添加工作区”选择本地目录。TSV 使用 `FileSystemDirectoryHandle` 的 `read` 模式，仅递归建立 CSV、Parquet 和 PQ 文件的相对路径索引；不会申请写权限。数据文件配置使用模糊路径选择器从该索引中选择文件，随后自动读取和缓存，无需额外的“绑定”动作。目录句柄只保留在当前浏览器会话内，刷新页面后需要重新选择工作区。
